...
with open(args[0], 'r') as f:
    for line in f:
        ...
        if "allocation" in j:
            ...
            as_type = AS["type"]
            if as_type not in AddrSpaces:
                AddrSpaces[as_type] = AddressSpace(as_type)

            if Id not in AddrSpaces[as_type].allocations:
                AddrSpaces[as_type].allocations += [Id]

            print "adding allocation", Id
            Allocations[Id] = Allocation(Id, pos, size, pagetype)
