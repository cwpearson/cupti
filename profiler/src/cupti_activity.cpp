...
template <typename T>
void insertDataHelper(std::map<std::string, std::string> &memcpyData, std::string key, T value){
...
static void handleCuptiKindKernel(CUpti_Activity *record) {
  ...
  std::map<std::string, std::string> kernelData;
...
static void handleCuptiKindMemcpy(CUpti_Activity *record) {
  ...
  //Extract useful data and send to KernelTime
  std::map<std::string, std::string> memcpyData;
