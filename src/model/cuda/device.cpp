...
Device::Device(const cudaDeviceProp &prop, int id) id_(id),
    unifiedAddressing_(prop.unifiedAddressing_),
    canMapHostMemory_(prop.canMapHostMemory),
    pageableMemoryAccess_(prop.pageableMemoryAccess),
    concurrentManagedAccess_(prop.concurrentManagedAccess_), major_(prop.major),
    minor_(prop.minor) {}