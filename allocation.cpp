...
std::string to_string(const Allocation::Location &l) {
  if (Allocation::Location::Host == l) return std::string("host");
  if (Allocation::Location::Device == l) return std::string("device");
...
std::pair<Allocations::map_type::iterator, bool> Allocations::insert(const Allocations::value_type &v) {
